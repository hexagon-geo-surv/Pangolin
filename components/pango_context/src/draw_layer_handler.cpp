...
{
  sophus::SE3d point_T_cam(sophus::Rotation3F64(), -axis.origin());
...
void cameraTranslatePointToPoint(MouseUpdateArgs& info)
{
  ...
  info.render_state.camera_from_world =
      sophus::SE3d(sophus::Rotation3F64(), to_in_from) *
      info.pointer_pressed.cam_T_world;
}
...
void cameraMoveTowardsPoint(MouseUpdateArgs& info, const double zoom_input)
{
  ...
  info.render_state.camera_from_world =
      sophus::SE3d(sophus::Rotation3F64(), vec) * info.render_state.camera_from_world;
}
...
class HandlerImpl : public DrawLayerHandler
{
      ...
      DrawLayerRenderState& render_state) override
  {
    ...
    Isometry3<double>& camera_from_world = render_state.camera_from_world;
    Similarity2<double>& clip_view_transform = render_state.clip_view_transform;

    auto camera_limits_in_world = Region3F64::unbounded();
